...
aws s3 cp $ZIP_NAME s3://$AWS_S3_BUCKET/$AWS_S3_FILEPATH 1>&2 || exit 1
...
rename() {
  aws lambda $CU-alias --function-name $AWS_LAMBDA_FUNC_NAME --name $1 --function-version $2 1>&2 || exit 1
}
